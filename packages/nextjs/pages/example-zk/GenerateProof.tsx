...
import { generateProof } from "~~/hooks/noir/useProofGenerator";
...
const buildNoirIntArray = (hexString: string) => {
  ...
  return trimmedHexString.split("").reduce((resultArray: string[], letter: string, index: number) => {
    if (index % 2 === 0) {
...
export const GenerateProof = ({ requiredBirthYear }: { requiredBirthYear: number }) => {
  ...
  const handleSubmission = async () => {
    ...
    await generateProof("LessThenSignedAge", parsedForm);
  };
  return (
    <>
